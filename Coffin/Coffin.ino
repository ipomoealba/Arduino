...
bool checkRFID(int i) {
  ...
  if (status == MI_OK)
  {
    ...
    uchar* id = serNum;
    //    8F0F7729
    if ( id[0] == 0x8F && id[1] == 0x0F && id[2] == 0x77 && id[3] == 0x29 && i == 1)
    {
      Serial.println();
    ...
    //if the card id is 7FFAC429,then relay connect
    else if (id[0] == 0x7F && id[1] == 0xFA && id[2] == 0xC4 && id[3] == 0x29 && i == 0)
    {
      Serial.println();
      Serial.println("isis");
      Serial.println();
    ...
    //    9D276B2D
    else if (id[0] == 0x9D && id[1] == 0x27 && id[2] == 0x6B && id[3] == 0x2D && i == 2)
    {
      Serial.println();
      Serial.println("anobis");
      Serial.println();
      return true;
    }
    else {
...
void getRequest(void)
{
  ...
  const char *data = "GET /?name=Coffin&STATUS=COMPLETE HTTP/1.1\r\n\r\n";
